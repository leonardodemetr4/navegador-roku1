        ...
        if value <> ""
            ...
            if Left(lower, 7) <> "http://" and Left(lower, 8) <> "https://"
                ...
                if Instr(1, value, ".") = 0
                    ...
                    value = "https://www.bing.com/search?q=" + value.EncodeUriComponent()
...
sub openPage()
    ...
    m.status.text = "Abrindo pagina..."

    requestUrl = m.serverBase + "/v3/open?session=" + m.session

    requestUrl = requestUrl + "&url=" + m.currentUrl.EncodeUriComponent()
...
sub clickPage()
    ...
    if px > 1279 then px = 1279
    if py > 719 then py = 719

    m.status.text = "Clicando..."

    requestUrl = m.serverBase + "/v3/click?session=" + m.session

    requestUrl = requestUrl + "&x=" + px.ToStr()

    requestUrl = requestUrl + "&y=" + py.ToStr()
...
sub goBack()
    ...
    m.status.text = "Voltando..."

    requestUrl = m.serverBase + "/v4/back?session=" + m.session
...
sub scrollPage(amount as Integer)
    ...
    if amount > 0
        ...
        m.status.text = "Rolando para baixo..."

    else

        m.status.text = "Rolando para cima..."

    end if

    requestUrl = m.serverBase + "/v4/scroll?session=" + m.session

    requestUrl = requestUrl + "&dy=" + amount.ToStr()
...
sub onImageReady(e as Object)
    ...
    if uri <> invalid and uri <> ""
        ...
        m.status.text = "Pagina carregada"
...
sub moveCursor(dx as Integer, dy as Integer)
    ...
    if m.cx < 35 then m.cx = 35
    if m.cx > 1225 then m.cx = 1225

    if m.cy < 120 then m.cy = 120
    if m.cy > 605 then m.cy = 605
